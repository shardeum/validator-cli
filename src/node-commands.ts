...
if (fs.existsSync(nodeConfigPath)) {
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename
  fs.writeFileSync(path.join(__dirname, `../${File.NODE_CONFIG}`), JSON.stringify(nodeConfig, undefined, 2), {
    encoding: 'utf8',
    mode: 0o600,
  })
}
...
if (process.env.EXT_IP) {
  config = merge(
    config,
    {
      server: {
        ip: {
          externalIp: process.env.EXT_IP === 'auto' ? process.env.SERVERIP : process.env.EXT_IP,
        },
      },
    },
    { arrayMerge: (target, source) => source }
...
if (process.env.INT_IP) {
  config = merge(
    config,
    {
      server: {
        ip: {
          internalIp: process.env.INT_IP === 'auto' ? process.env.SERVERIP : process.env.INT_IP,
        },
      },
    },
    { arrayMerge: (target, source) => source }
...
const dashboardPackageJson = JSON.parse(readFileSync(path.join(__dirname, '../../package.json'), 'utf8'))
...
export function registerNodeCommands(program: Command) {
  program
    .command('status')
    .description('Show if validator is running or not; also the port and URL to connect to it')
    .action(async () => {
      pm2.describe('validator', async (err, descriptions) => {
        ...
        if (descriptions.length === 0) {
          ...
          console.log(
            yaml.dump({
              ...
              exitStatus,
              performance,
              stakeRequirement: stakeRequired ? ethers.utils.formatEther(stakeRequired) : '',
              lockedStake: accountInfo.lockedStake ? ethers.utils.formatEther(accountInfo.lockedStake) : '',
        ...
        if (status.status !== 'stopped') {
          ...
          if (nodeStatus === 'initializing')
            nodeStatus = lockedStakeStr === '0.0' ? 'need-stake' : 'waiting-for-network'
          ...
          console.log(
            yaml.dump({
              ...
              performance,
              currentRewards: ethers.utils.formatEther(accountInfo.accumulatedRewards.toString()),
        ...
        console.log(
          yaml.dump({
            ...
            currentRewards: accountInfo
              ? ethers.utils.formatEther(accountInfo.accumulatedRewards.toString())
              : '',
        ...
        console.log(
          yaml.dump({
            stake: eoaData?.operatorAccountInfo
              ? ethers.utils.formatEther(String(parseInt(eoaData.operatorAccountInfo.stake, 16)))
              : '',
  ...
  program
    ...
    .action(() => {
      ...
      execFile(`node`, [`${path.join(__dirname, VALIDATOR_CLEAN_PATH)}`], () => {
        ...
        pm2.connect((err) => {
          ...
          const scriptPath = path.join(__dirname, `../../${File.ENVIRONMENT_CONFIG}`)
            ...
            pm2.describe('validator', (err, processDescription) => {
              ...
              if (processDescription && processDescription.length > 0) {
                ...
                if (processInfo.pm2_env?.status === 'online') {
                  ...
                  throw new Error(`The process is not running. Status: ${processInfo.pm2_env?.status}`)
                }
              } else {
      ...
      pm2.connect((err) => {
        ...
        pm2.describe('validator', async (err, descriptions) => {
          ...
          if (!options.force) {
            ...
            if (nodeInfo != null && (nodeInfo.status === 'syncing' || nodeInfo.status === 'active')) {
              const answer = await getUserInput(
    ...
    .action(async (stakeValue) => {
      ...
      try {
        ...
        const txDetails = {
          ...
          gasLimit: 30000000,
          value,
          data: ethers.utils.hexlify(ethers.utils.toUtf8Bytes(JSON.stringify(stakeData))),
          nonce,
        }
  ...
  async function unstake(options: { force: boolean }) {
    ...
    try {
      ...
      if (eoaData.operatorAccountInfo?.nominee == null || eoaData.operatorAccountInfo?.stake === '00') {
  ...
  program
    ...
    .action(() => {
      exec('sh update.sh', { cwd: path.join(__dirname, '../..') }, (error, stdout, stderr) => {
        console.log(stdout)
        console.log(stderr)
        if (error != null) {
      ...
      exec('sh update.sh', { cwd: path.join(__dirname, '../../../gui') }, (error, stdout, stderr) => {
        console.log(stdout)
        console.log(stderr)
        if (error != null) {
          console.log(`exec error: ${error}`)
  ...
  program
    .command('version')
    .description('Shows the installed version, latest version and minimum version of the operator dashboard')
    .action(async () => {
  ...
  program
    ...
    .description('Show statistics like TPS, active nodes etc. about the network')
    .action(async () => {
      pm2.describe('validator', async (err, [descriptions]) => {
        if (!err) {
  ...
  const setCommand = program.command('set').description('command to set various config parameters')
  ...
  setCommand
    ...
    .argument('<true/false>')
    .description('To autostart the node after being rotated out. Set autostart to true or false')
